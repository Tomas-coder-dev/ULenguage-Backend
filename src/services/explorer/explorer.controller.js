...
// cache para place details (reduce llamadas a Place Details)
...
function setCachedPlaceDetails(placeId, value) {
  placeDetailsCache.set(placeId, { value, expiresAt: Date.now() + DESCRIPTION_CACHE_TTL_MS });
}
...
function buildPhotoUrl(photoReference, maxWidth = 800) {
  ...
  return `https://maps.googleapis.com/maps/api/place/photo?maxwidth=${maxWidth}&photoreference=${encodeURIComponent(photoReference)}&key=${GOOGLE_PLACES_API_KEY}`;
}
...
async function fetchPlaceDetails(placeId) {
  ...
  if (!GOOGLE_PLACES_API_KEY) {
    // cannot fetch remote details without API key
    return null;
  }
  ...
  try {
    ...
    // Build photos array (url) using photo_reference if exists
    let photos = [];
    ...
    const details = {
      ...
      opening_hours: result.opening_hours || null, // includes open_now + weekday_text
    ...
    const typesLower = (details.types || []).map(t => String(t).toLowerCase());
    const nameVicinity = `${details.name || ''} ${details.vicinity || ''}`.toLowerCase();
    details.has_parking = false;
    if (typesLower.includes('parking')) details.has_parking = true;
    if (/\b(parking|estacionamiento|cochera|garage|garaje)\b/i.test(nameVicinity)) details.has_parking = true;
    // Note: esto es heurístico; no garantiza resultado correcto.

    // Cache it
    setCachedPlaceDetails(placeId, details);
    return details;
  } catch (err) {
    console.warn('[ExplorerService] fetchPlaceDetails error for', placeId, err?.response?.status || err?.message || err);
    return null;
  }
}
 ...
 *
 * Supports:
 *  - source=db  => returns Zones from MongoDB (Zone collection)
 *  - source=places (default) => uses Google Places (requires GOOGLE_PLACES_API_KEY)
 *
 * Common query params:
 *  - query: text search (only applied to DB source)
 *  - types: comma-separated Google Place types (lodging,restaurant,...)
 *  - location: lat,lng (places)
 *  - radius: meters (places)
 *  - details=true & details_count=n : fetch Place Details for top n places (places)
 */
async function getPlaces(req, res) {
  try {
    const source = (req.query.source || '').toString().toLowerCase(); // 'db' or 'places'
    const query = req.query.query ? req.query.query.toString().toLowerCase() : null;

    // If user asked explicitly for DB or GOOGLE key missing -> use Zone DB
    if (source === 'db' || !GOOGLE_PLACES_API_KEY) {
      ...
      const places = zones.map(zone => ({
        ...
        // DB cannot guarantee place details; keep fields consistent
    ...
    // GOOGLE PLACES flow (default)
    // Location defaults (Cusco)
    let lat = -13.53195;
    let lng = -71.967463;
    ...
    if (types.length === 0) types.push('tourist_attraction'); // default
    const pageToken = req.query.page_token ? `&pagetoken=${encodeURIComponent(req.query.page_token)}` : '';
    const wantDetails = (String(req.query.details || 'false').toLowerCase() === 'true');
    const detailsCount = parseInt(req.query.details_count, 10) || 5;

    // One request per type
    const requests = types.map(type => {
      const url = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${lat},${lng}&radius=${radius}&type=${encodeURIComponent(type)}&key=${GOOGLE_PLACES_API_KEY}${pageToken}`;
      return axios.get(url, { timeout: 8000 }).then(r => ({ type, data: r.data })).catch(err => ({ type, error: err }));
    });
    ...
    for (const respWrap of responses) {
      if (respWrap.status !== 'fulfilled') {
        console.warn('[ExplorerService] Places request failed:', respWrap.reason?.message || respWrap.reason);
        continue;
      }
      const resp = respWrap.value;
      if (resp && resp.error) {
        console.warn('[ExplorerService] Places request error for type=%s: %o', resp.type, resp.error?.response?.status || resp.error?.message || resp.error);
        continue;
      }
      const data = resp.data || {};
      const results = Array.isArray(data.results) ? data.results : [];
      combinedResults = combinedResults.concat(results);
      if (!nextPageToken && data.next_page_token) nextPageToken = data.next_page_token;
    }

    // Deduplicate
    const uniquePlaces = dedupePlaces(combinedResults);

    // Precompute descriptions (IA) for first maxIaDescriptions places
    const maxIaDescriptions = 6;
    const placesToDescribe = uniquePlaces.slice(0, maxIaDescriptions);

    await Promise.allSettled(placesToDescribe.map(async (place) => {
      try {
        ...
        if (!description) {
          try {
            ...
            if (raw && typeof raw === 'object') {
              description = {
                es: (raw.es && String(raw.es).trim()) || fallbackEs,
                en: (raw.en && String(raw.en).trim()) || fallbackEn,
                qu: (raw.qu && String(raw.qu).trim()) || fallbackQu
              };
            } else {
              description = { es: fallbackEs, en: fallbackEn, qu: fallbackQu };
            }
            setCachedDescription(placeKey, description);
          } catch (err) {
            console.warn('[ExplorerService] getPlaceDescriptionIA failed for %s: %s', place.name, err?.message || err);
            description = { es: fallbackEs, en: fallbackEn, qu: fallbackQu };
            setCachedDescription(placeKey, description);
          }
        }
      } catch (e) {
        console.warn('[ExplorerService] describe place error:', e?.message || e);
      }
    }));

    // If the caller requested full details, fetch Place Details for top detailsCount places
    let detailsMap = new Map();
    if (wantDetails) {
      const toDetails = uniquePlaces.slice(0, detailsCount);
      const detailsResults = await Promise.allSettled(toDetails.map(p => fetchPlaceDetails(p.place_id)));
    ...
    const payloadPlaces = uniquePlaces.map((place) => {
      const placeKey = (place.place_id || place.name || `${place.geometry?.location?.lat},${place.geometry?.location?.lng}`).toString();
      const description = getCachedDescription(placeKey) || {
        es: place.vicinity || place.name || 'Descripción no disponible por el momento.',
        en: place.vicinity || place.name || 'Description not available at the moment.',
    ...
    return res.json({ places: payloadPlaces, next_page_token: nextPageToken || null });
  } catch (error) {
    console.error('[ExplorerService][ERROR]', error?.response?.status || error?.message || error);
...
async function getPlaceDetailsEndpoint(req, res) {
  try {
    ...
    if (source === 'db') {
      ...
      if (!id) return res.status(400).json({ message: "Falta parámetro 'id' para source=db" });
      const zone = await Zone.findById(id).select('-__v -created_at -updated_at -qr_code').lean();
    ...
    if (!placeId) return res.status(400).json({ message: "Falta parámetro 'place_id'." });

    const details = await fetchPlaceDetails(placeId);
    if (!details) return res.status(404).json({ message: 'Detalles no disponibles para este place_id.' });

    return res.json(details);
  } catch (err) {
    console.error('[ExplorerService][DETAILS][ERROR]', err?.message || err);
    return res.status(500).json({ message: 'Error obteniendo detalles.' });
  }
}
