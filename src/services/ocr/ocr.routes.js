...
const {
  analyzeAndExplain,
  analyzeExplainAndTranslate,
  getOcrStatus,
  extractTextAndTranslate, // <- nuevo endpoint simple
...
// Ruta principal que usa Vision y Gemini (objetos + explicaciones culturales)
router.post('/analyze', upload.single('image'), analyzeAndExplain);

// Ruta que además traduce el resultado completo (mantener compatibilidad)
router.post(
...
// NUEVA RUTA: OCR simple para el traductor (solo texto + traducción)
router.post(
...
// const { extractText, extractTextAutoLang, extractTextAndTranslateLegacy } = require('./ocr.controller');
...
// router.post('/extract-text-and-translate-legacy', upload.single('image'), extractTextAndTranslateLegacy);
