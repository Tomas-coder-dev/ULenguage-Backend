...
/**
 * Helper: ejecuta una promesa con timeout y devuelve:
 * { ok: boolean, value: any, error?: any, timeout?: boolean }
 */
async function withTimeout(promise, ms = DEFAULT_TIMEOUT_MS, fallback = null) {
  ...
  const timeoutP = new Promise((resolve) => {
    timer = setTimeout(
      () => resolve({ ok: false, timeout: true, value: fallback }),
      ms
    );
  });
...
function simplePlaceFallback(place) {
  ...
  if (Array.isArray(place.types) && place.types.length) {
    parts.push(`Tipo: ${place.types.slice(0, 3).join(', ')}`);
  }
 ...
 *
 * Ajustes:
 * - Para quechua usa translateTextHybrid (BD QuechuaCusqueno → Glosbe → Google).
 */
async function getPlaceDescriptionIA(placeOrName) {
  // Si GEMINI no está configurado, usamos solo fallback + traducciones
  if (!GEMINI_API_KEY) {
    warn('getPlaceDescriptionIA - GEMINI_API_KEY no configurada; devolviendo fallback.');
    const fallbackEs =
      simplePlaceFallback(placeOrName) || 'Descripción no disponible por configuración.';

    const [en, qu] = await Promise.all([
      (async () => {
        try {
          return await translateTextGoogle(fallbackEs, 'en');
        } catch {
      ...
      (async () => {
        try {
          // usa flujo híbrido a quechua general/cusqueño
          return await translateTextHybrid(fallbackEs, 'es', 'quz');
        } catch {
          return 'Manaraq kashanmi willakuy.';
        }
      })(),
    ]);
    ...
    return {
      es: fallbackEs,
      en: en || 'Description not available.',
      qu: qu || 'Manaraq kashanmi willakuy.',
    };
  }

  const place =
    typeof placeOrName === 'string' ? { name: placeOrName } : placeOrName || {};
  const name = place.name || place.place_name || '';
  ...
  // Ajustar instrucciones según tipo
  let styleHint = '';
  if (
    typesArr.some((t) =>
      ['museum', 'art_gallery', 'tourist_attraction', 'historic', 'point_of_interest'].includes(
        t
      )
    )
  ) {
    styleHint =
      'Escribe con foco en el contexto cultural, historia breve y relevancia local. Prioriza información cultural y patrimonio.';
  } else if (
    typesArr.some((t) =>
      ['cafe', 'restaurant', 'bar', 'bakery', 'food'].includes(t)
    )
  ) {
    styleHint =
      'Describe el ambiente, ofertas típicas y la mejor hora para visitar; menciona aspectos prácticos (ej. ideal para desayuno, tarde, etc.).';
  } else {
    styleHint =
      'Describe qué lo hace interesante o útil para un visitante local o turista, en un tono claro y conciso.';
  }
  ...
  try {
    ...
    if (respWrap.timeout) {
      warn(
        'getPlaceDescriptionIA - Gemini request timed out after %dms for "%s"',
        duration,
        name || 'N/A'
      );
    } else if (!respWrap.ok) {
      ...
      if (e?.response) {
        error(
          'getPlaceDescriptionIA - Gemini HTTP error status=%d body=%s',
          e.response.status,
          util.inspect(e.response.data, { depth: 6 })
        );
      } else {
      ...
      info(
        'getPlaceDescriptionIA - Gemini responded in %dms for "%s"',
        duration,
        name || 'N/A'
      );
    }
    ...
    error(
      'getPlaceDescriptionIA - unexpected error: %s',
      err?.message || util.inspect(err, { depth: 4 })
    );
    description_es = '';
  }
  ...
  if (!description_es) {
    description_es =
      simplePlaceFallback(place) || 'Descripción no disponible por el momento.';
    info(
      'getPlaceDescriptionIA - using fallback for "%s": %s',
      name || 'N/A',
      description_es
    );
  }
  ...
  try {
    const trEn = await translateTextGoogle(description_es, 'en');
    if (trEn && trEn.trim()) {
      description_en = sanitizeAndLimit(trEn, MAX_PLACE_CHARS);
    }
  } catch (e) {
    warn('getPlaceDescriptionIA - translate en failed: %s', e?.message || e);
  }

  try {
    // Híbrido: es -> quz (quechua cusqueño/general)
    const trQu = await translateTextHybrid(description_es, 'es', 'quz');
    if (trQu && trQu.trim()) {
      description_qu = sanitizeAndLimit(trQu, MAX_PLACE_CHARS);
    }
  } catch (e) {
    warn('getPlaceDescriptionIA - translate qu failed: %s', e?.message || e);
  }
...
async function getCulturalExplanation(text, labels = [], objects = [], targetLang = 'es') {
  ...
  if (!hasContent) {
    ...
    try {
      ...
      if (trWrap && trWrap.ok && trWrap.value) {
        return {
          explanation: sanitizeAndLimit(trWrap.value, MAX_EXPLANATION_CHARS),
          lang,
        };
      }
      if (typeof trWrap === 'string' && trWrap) {
        return {
          explanation: sanitizeAndLimit(trWrap, MAX_EXPLANATION_CHARS),
          lang,
        };
      }
    } catch (e) {
  ...
  const labelsStr =
    Array.isArray(labels) && labels.length ? labels.join(', ') : 'Ninguno';
  const objectsStr =
    Array.isArray(objects) && objects.length
...
Sé informativo, conciso y evita juicios políticos o aseveraciones no verificadas.
  ...
  try {
    if (!GEMINI_API_KEY) {
      warn(
        'getCulturalExplanation - GEMINI_API_KEY no configurada; se intentará fallback de traducción si es posible.'
      );
    } else {
      const start = Date.now();
      const axiosPromise = axios.post(
      ...
      if (respWrap.timeout) {
        ...
        if (e?.response) {
          error(
            'getCulturalExplanation - Gemini HTTP error status=%d body=%s',
            e.response.status,
            util.inspect(e.response.data, { depth: 6 })
          );
        } else {
          error(
            'getCulturalExplanation - Gemini error: %s',
        ...
        info(
          'getCulturalExplanation - Gemini responded in %dms but text was empty/insufficient',
          duration
        );
      }
    }
    ...
    error(
      'getCulturalExplanation - unexpected Gemini error: %s',
      err?.message || util.inspect(err, { depth: 6 })
    );
  }
  ...
  try {
    if (GEMINI_API_KEY) {
      ...
      if (respEsWrap.ok && respEsWrap.value) {
        let geminiEs =
          respEsWrap.value?.data?.candidates?.[0]?.content?.parts?.[0]?.text || '';
        geminiEs = sanitizeAndLimit(geminiEs, MAX_EXPLANATION_CHARS);
        if (geminiEs) {
          const trWrap = await withTimeout(
            translateTextGoogle(geminiEs, lang),
            8000,
            null
          );
          if (trWrap && trWrap.ok && trWrap.value) {
            ...
            return {
              explanation: sanitizeAndLimit(geminiEs, MAX_EXPLANATION_CHARS),
              lang,
            };
          }
        }
        ...
        warn(
          'getCulturalExplanation (es fallback) - Gemini timed out after %dms',
          durEs
        );
        ...
        if (e?.response) {
          error(
            'getCulturalExplanation (es fallback) - Gemini HTTP error status=%d body=%s',
            e.response.status,
            util.inspect(e.response.data, { depth: 6 })
          );
        } else {
          error(
            'getCulturalExplanation (es fallback) - Gemini error: %s',
            util.inspect(e, { depth: 6 })
          );
        }
      }
    ...
    warn(
      'getCulturalExplanation - fallback es unexpected error: %s',
      e?.message || util.inspect(e, { depth: 4 })
    );
  }

  // 3) Último recurso: traducir mensaje por defecto
  try {
    const translatedDefaultWrap = await withTimeout(
      translateTextGoogle(fallbackBaseEs, lang),
      6000,
      null
    );
    if (translatedDefaultWrap && translatedDefaultWrap.ok && translatedDefaultWrap.value) {
      return {
        explanation: sanitizeAndLimit(
          translatedDefaultWrap.value,
      ...
      typeof translatedDefaultWrap === 'string' &&
      translatedDefaultWrap
    ) {
      return {
        explanation: sanitizeAndLimit(
          translatedDefaultWrap,
